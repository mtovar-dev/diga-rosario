 ...
 * @author MITM
 */
public class Fxml_Aud_Gh_EmpresaController implements Initializable {
    ...
    private static final String ScreenName = "Aud. x Empresa";
...
//        for (int i = localCalendar.get(Calendar.MONTH) + 1; i >= 1; i--) {
     ...
     */
    private void showInfoEmpresaAno(Empresa empresa, String nomina, Integer year, String month, int type){
        Integer imonth = 0;
        switch (month){
            case "Enero":
                imonth = 1;
            ...
            case "Diciembre":
                ...
                break;
            default:
                imonth = cb_mes.getSelectionModel().getSelectedIndex();
        }

        //Obtiene y llena las tablas con los resultados
        Asignacion[] asigna = Ln.getInstance().loadAsignacionesEmpresaXProceso(empresa, nomina, year, imonth, type);   
        if(asigna != null){  
        ...
        Deduccion[] deduc = Ln.getInstance().loadDeduccionesEmpresaXProceso(empresa, nomina, year, imonth, type);   
        if(deduc != null){  
    ...
    private void init_FocusArray(int opc){
        ...
        switch(opc){
            case 0:     //SOLO LECTURA
                nodos = new Node[]{
                    cb_empresa, cb_ano, cb_mes, cb_nomina, cb_proceso};
                break;
            case 1:     //NUEVO
                nodos = new Node[]{
                    cb_empresa, cb_ano, cb_mes, cb_nomina, cb_proceso};
    ...
    private void init_buttons(){
         ...
         */
        cb_empresa.getSelectionModel().selectedItemProperty().addListener((ObservableValue<? extends Empresa> arg0, Empresa arg1, Empresa arg2) -> {
            if (arg2 != null)
                loadComboboxClear();
        });
        /**
        ...
        cb_mes.getSelectionModel().selectedItemProperty().addListener((ObservableValue<? extends String> arg0, String arg1, String arg2) -> {
            if (arg2 != null)
                showInfoEmpresaAno(cb_empresa.getValue(), cb_nomina.getValue(), cb_ano.getValue(), "", cb_proceso.getValue());
        });
        /**
         * metodo para mostrar el item (estado) seleccionado y cargar las ciudades
         * param: nombre del estado
        ...
        cb_nomina.getSelectionModel().selectedItemProperty().addListener((ObservableValue<? extends String> arg0, String arg1, String arg2) -> {
            if (arg2 != null)
                loadProcesosXMes(cb_empresa.getValue(), cb_nomina.getValue(), cb_ano.getValue(), "Enero");
