 ...
 * @author MITM
 */
public class Fxml_RoleController implements Initializable {
    ...
    @FXML //  fx:id="tb_table"
    private TableView<Rol> tb_table; // Value injected by FXMLLoader
...
//    private ObservableList<Rol> role;
    private static Integer[] toolsConfig; 
    private static String[] tooltips;
    
    private static final String ScreenName = "Rol";
...
//               }
    ...
    */
    private void createTable(){
        ...
        this.objectWidth(col_rol        , 80, 120);
        ...
        col_status.setCellFactory(new Callback<TableColumn, TableCell>() {
            ...
            public TableCell call(TableColumn param) {
                return new TableCell<Rol, Object>() {
                    @Override
        ...
        //Se define la columna de la tabla con el nombre del atributo del objeto ROLE correspondiente
    ...
    private boolean saveRole() {
        ...
        String rolename = tf_nombre.getText();
        ...
        if(rolename != null && !rolename.equals("")){
            ...
            Rol role = new Rol();
            role.setIdRol(Integer.parseInt(tf_id.getText()));
            role.setNombre(tf_nombre.getText());
            role.setAbrev(tf_abrev.getText());
            role.setStatus(Datos.getRole().getStatus());      //Se asigna el STATUS del usuario
            //Se llama al proceso de Guardado
            boolean result = 
                    Ln.getInstance().save_Rol(role, proceso, ScreenName);
            //Si el Resultado es correcto
            if(result){
                //Se Notifica al usuario
                Gui.getInstance().showMessage("El " + ScreenName + " se ha Guardado Correctamente!", "I");
                loadRoles();            //Se Recarga la tabla de usuarios existentes
                return true;
            }     
        }else{                            
    ...
    private void refreshForm(){        
        tf_id.setText(String.valueOf(Datos.getRole().getIdRol()));
        tf_nombre.setText(Datos.getRole().getNombre());
        tf_abrev.setText(Datos.getRole().getAbrev());
...
//                lb_Title.setText(Tools.verticalText("EDITAR"));
    ...
    private void loadRoles(){        
        Datos.setRep_role(Ln.getInstance().load_Rol());
        loadTable( Datos.getRep_role());     
    }
    /**
     * Procedimiento de llenado de datos en la tabla de datos
     */
    private void loadTable(Rol[] role){    
        if(role != null){
            ObservableList<Rol> data = FXCollections.observableArrayList();        
            data.addAll(Arrays.asList(role));        
    ...
    private void selectedRow(){
        ...
        Rol role = tb_table.getSelectionModel().getSelectedItem();          
        if(role != null){
            Datos.setRole(role);          //Asigno el Usuario a la Clase de Datos Globales
            change_im_tool4(role.getStatus());  //Se define el valor del Boton de Cambio se Status
    ...
    private void defineToolBar() {
        ...
        //CARGA DE LA BD LA CONFIGURACION DE ROLE PARA LA PANTALLA
            ...
            public void handle(KeyEvent ke){
                ...
                if (ke.getCode().equals(KeyCode.ENTER) || ke.getCode().equals(KeyCode.TAB)){     
                    ...
                    if(((Node)ke.getSource()).getId().equals("tf_buscar")){                        
                        try{
                            Datos.setRep_role(Ln.getInstance().find_Rol(tf_buscar.getText()));
                            loadTable(Datos.getRep_role());     
                        } catch(Exception e){
    ...
    private void botonInicio() {
        ...
        Datos.setRole(new Rol());                           
        refreshForm();                      
        Datos.setRole(null);             //RESET DE LA VARIABLE
    ...
    private void botonEditar(){
        if(Datos.getRole()!= null && toolsConfig[3]==1){
            tipoOperacion = 2;
            change_im_tool4(Datos.getRole().getStatus());
            refreshForm();
            setFormVisible(true);     
            Gui.getFields()[Gui.getFieldFocused()].requestFocus();
        }
    ...
    private void botonGuardar(){        
        if(Datos.getRole()!= null && toolsConfig[4]==1){
            boolean result = saveRole();
            if (result)
                botonInicio();
        }
    }
    ...
    private void botonEliminar() {
        if(Datos.getRole()!= null && toolsConfig[5]==1){
    ...
    private void botonOcultar(){
        ...
        Datos.setRole(new Rol());                           
        refreshForm();                      
        Datos.setRole(new Rol());    //RESET DE LA VARIABLE
        setFormVisible(false);              //OCULTA EL FORMULARIO
    }
    /**
     * 
     ...
     * @param boo TRUE si el ROLE esta validado
     */
    private void change_im_check(boolean boo){
        if(boo){    //IMAGEN SI EL ROLE ES CORRECTO
            im_check.setImage(new Image(getClass().getResourceAsStream("/Images/img06.png")));
        }else{      //IMAGEN PARA LA BUSQUEDA DE UN ROLE
    ...
    private void change_im_val(int boo){
        // 200 HTTP_request Ok
        if(boo == 200){    //IMAGEN SI EL ROLE ES CORRECTO
    ...
    private void change_im_tool4(int boo){
        if(boo == 0){   //IMAGEN SI EL ROLE ESTA INHABILITADO
            im_tool4.setImage(new Image(getClass().getResourceAsStream("/Images/img07.png")));
        }else{          //IMAGEN PARA HABILITAR AL ROLE
